...
int BajaPersona(ePersona arrayDePersonas[], int tam)
{
    ...
    if(arrayDePersonas!=NULL && tam>0)
    {
        mostrarPersonas(arrayDePersonas, tam);
...
1) Buscar lugar (con buscar libre que te devuelve el indice del lugar libre o -1 si no hay lugar y se informa que no hay lugar)
2) Pido datos y los guardo en el indice libre y el isEmpty cambia a 0 (isEmpty=0 -> esta ocupado el indice)
*/

/*
BAJA:
Pido id llamo a buscar persona y devuelve el indice donde esta esa persona o que devuelva un -1 y informo que no existe ese id si no esta con id 0

si existe muestro la persona con la funcion mostrar persona y le pido confirmacion de baja

si confirma insenti en 1 y se completa la baja
*/

/*
MODIFICAR PERSONA:

lo mismo que baja (hasta lo de encontrar la perdsona )pero se hace un sub menu que muestre: que quiere modificar??? el sexo? la altura?
*/
